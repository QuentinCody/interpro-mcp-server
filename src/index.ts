...
import { InterproDataDO } from "./do";

export { InterproDataDO };

interface InterproEnv {
...
export class MyMCP extends StatelessMcpWorker {
    ...
    async init() {
        ...
        const env = this.env as unknown as InterproEnv;
...
export default {
    fetch(request: Request, env: Env, ctx: ExecutionContext) {
        ...
        if (url.pathname === "/health") {
            return buildHealthResponse("interpro");
        }
        ...
        if (url.pathname === "/readyz") {
            ...
            return MyMCP.readiness(env, "interpro");
        }
